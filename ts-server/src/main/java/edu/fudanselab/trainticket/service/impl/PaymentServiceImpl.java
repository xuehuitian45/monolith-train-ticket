...
public class PaymentServiceImpl implements PaymentService {
    ...
    public Response pay(Payment info, HttpHeaders headers){
        // 验证必填字段
        if (info == null) {
            LOGGER.error("[pay][Payment info is null]");
            return new Response<>(0, "Payment info is null", null);
        }

        if (info.getOrderId() == null || info.getOrderId().isEmpty()) {
            LOGGER.error("[pay][OrderId is null or empty]");
            return new Response<>(0, "OrderId is required", null);
        }

        List<Payment> existingPayments = paymentRepository.findByOrderId(info.getOrderId());
