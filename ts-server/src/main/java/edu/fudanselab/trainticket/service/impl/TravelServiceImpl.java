...
public class TravelServiceImpl implements TravelService {
    ...
    public Response create(TravelInfo info, HttpHeaders headers) {
        // 验证必填字段
        if (info == null) {
            LOGGER.error("[create][Create trip error][TravelInfo is null]");
            return new Response<>(0, "TravelInfo is null", null);
        }

        if (info.getTripId() == null || info.getTripId().isEmpty()) {
            LOGGER.error("[create][Create trip error][TripId is null or empty]");
