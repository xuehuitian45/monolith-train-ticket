...
public class PriceServiceImpl implements PriceService {
    ...
    public Response createNewPriceConfig(PriceConfig createAndModifyPriceConfig, HttpHeaders headers) {
        ...
        // 验证必填字段
        if (createAndModifyPriceConfig == null) {
            LOGGER.error("[createNewPriceConfig][Create price config error][PriceConfig is null]");
            return new Response<>(0, "PriceConfig is null", null);
        }

        if (createAndModifyPriceConfig.getRouteId() == null || createAndModifyPriceConfig.getRouteId().isEmpty()) {
            LOGGER.error("[createNewPriceConfig][Create price config error][RouteId is null or empty]");
            return new Response<>(0, "RouteId is required", null);
        }

        if (createAndModifyPriceConfig.getTrainType() == null || createAndModifyPriceConfig.getTrainType().isEmpty()) {
            LOGGER.error("[createNewPriceConfig][Create price config error][TrainType is null or empty]");
            return new Response<>(0, "TrainType is required", null);
        }

        PriceConfig priceConfig = null;
        // create
        if (createAndModifyPriceConfig.getId() == null || createAndModifyPriceConfig.getId().toString().length() < 10) {
            // 检查是否已存在相同的 routeId 和 trainType 组合
            PriceConfig existingConfig = priceConfigRepository.findByRouteIdAndTrainType(
                    createAndModifyPriceConfig.getRouteId(),
                    createAndModifyPriceConfig.getTrainType());
            if (existingConfig != null) {
                LOGGER.warn("[createNewPriceConfig][Create price config warn][Price config already exists][RouteId: {}, TrainType: {}]",
                        createAndModifyPriceConfig.getRouteId(), createAndModifyPriceConfig.getTrainType());
                return new Response<>(0, "Price config already exists for route " + createAndModifyPriceConfig.getRouteId() +
                        " and train type " + createAndModifyPriceConfig.getTrainType(), existingConfig);
            }
            ...
            try {
                ...
                LOGGER.error("[createNewPriceConfig][Create price config error][Data integrity violation][RouteId: {}, TrainType: {}]",
                        createAndModifyPriceConfig.getRouteId(), createAndModifyPriceConfig.getTrainType(), e);
                return new Response<>(0, "Price config already exists for route " + createAndModifyPriceConfig.getRouteId() +
                        " and train type " + createAndModifyPriceConfig.getTrainType(), null);
            }
        } else {
            // modify
            ...
            try {
                ...
                LOGGER.error("[createNewPriceConfig][Update price config error][Data integrity violation][RouteId: {}, TrainType: {}]",
                        createAndModifyPriceConfig.getRouteId(), createAndModifyPriceConfig.getTrainType(), e);
                return new Response<>(0, "Price config already exists for route " + createAndModifyPriceConfig.getRouteId() +
                        " and train type " + createAndModifyPriceConfig.getTrainType(), null);
            }
