...
public class InsidePaymentServiceImpl implements InsidePaymentService {
    ...
    public Response pay(PaymentInfo info, HttpHeaders headers) {
        // 验证必填字段
        if (info == null) {
            LOGGER.error("[Inside Payment Service.pay][PaymentInfo is null]");
            return new Response<>(0, "PaymentInfo is null", null);
        }

        if (info.getOrderId() == null || info.getOrderId().isEmpty()) {
            LOGGER.error("[Inside Payment Service.pay][OrderId is null or empty]");
            return new Response<>(0, "OrderId is required", null);
        }

        if (info.getTripId() == null || info.getTripId().isEmpty()) {
            LOGGER.error("[Inside Payment Service.pay][TripId is null or empty]");
            return new Response<>(0, "TripId is required", null);
        }
        ...
        try {
            reGetOrderResults = restTemplate.exchange(
                    requestOrderURL,
                    HttpMethod.GET,
            ...
            LOGGER.error("[Inside Payment Service.pay][Get order failed][orderId: {}][error: {}]", info.getOrderId(), e.getMessage(), e);
            return new Response<>(0, "Payment Failed, Cannot get order: " + e.getMessage(), null);
        }

        if (result == null) {
            LOGGER.error("[Inside Payment Service.pay][Payment failed][Order response is null][orderId: {}]", info.getOrderId());
            return new Response<>(0, "Payment Failed, Order response is null", null);
        }

        if (result.getStatus() == 1) {
            Order order = result.getData();
            if (order == null) {
                LOGGER.error("[Inside Payment Service.pay][Payment failed][Order data is null][orderId: {}]", info.getOrderId());
                return new Response<>(0, "Payment Failed, Order data is null", null);
            }
